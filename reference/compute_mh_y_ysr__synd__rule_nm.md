...
Computes the summary score `mh_y_ysr__synd__rule_nm` Youth Self Report
\[Youth\] (Syndrome Scale - Rule breaking behavior): Number missing
...
`vars_mh_y_ysr__synd__rule` is vector of all column names used to
compute summary score of `mh_y_ysr__synd__rule` scores.
