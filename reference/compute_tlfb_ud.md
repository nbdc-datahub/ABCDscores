...
Computes the number of use days for a given (set of) substance(s).
Optionally, allows to filter by period (detailed and/or estimated); only
considering a specified number of days before the TLFB interview; only
considering specific day types (weekends or week days); only considering
