...
Computes the summary score `mh_p_asr__dsm__dep_nm` Adult Self Report
\[Parent\] (DSM-5 Oriented Scale - Depresssive problems): Number missing
...
`vars_mh_p_asr__dsm__dep` is vector of all column names used to compute
summary score of `mh_p_asr__dsm__dep` scores.
