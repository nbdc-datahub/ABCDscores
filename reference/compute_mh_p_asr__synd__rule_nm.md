...
Computes the summary score `mh_p_asr__synd__rule_nm` Adult Self Report
\[Parent\] (Syndrome Scale - Rule breaking behavior): Number missing
...
`vars_mh_p_asr__synd__rule` is vector of all column names used to
compute summary score of `mh_p_asr__synd__rule` scores.
